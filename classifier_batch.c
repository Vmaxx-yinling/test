...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    while(get_current_batch(net) < net.max_batches || net.max_batches == 0){
        ...
        if(*net.seen/N > epoch){
            ...
            validate_classifier_single(datacfg, cfgfile, buff);//made change here
        }
...
void predict_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filelist, int top, char *outfile)
{
    network net = parse_network_cfg(cfgfile);
    if(weightfile){
        load_weights(&net, weightfile);
    ...
    FILE * in;
    in = fopen(filelist, "r");
    while(fgets(filename,128,in) != NULL){
        if(strlen(filename) == 1 ||strlen(filename) == 0)
            continue;
        strtrim(filename);
        ...
        fputs(names[indexes[0]], ofile);
        fputs(" ", ofile);
        sprintf(prob_str, "%f", predictions[indexes[0]]);
        fputs(prob_str, ofile);
        fputs("\n",ofile);
    }
    fclose(ofile);
...
void main(int argc, char **argv)
{
    ...
    char *layer_s = (argc > 6) ? argv[6]: 0;
    int layer = layer_s ? atoi(layer_s) : -1;
    if(0==strcmp(argv[1], "predict")) predict_classifier(data, cfg, weights, filename, top, outfile);
    else if(0==strcmp(argv[1], "try")) try_classifier(data, cfg, weights, filename, atoi(layer_s));
    else if(0==strcmp(argv[1], "train")) train_classifier(data, cfg, weights, gpus, ngpus, clear);
    else if(0==strcmp(argv[1], "demo")) demo_classifier(data, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[1], "gun")) gun_classifier(data, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[1], "threat")) threat_classifier(data, cfg, weights, cam_index, filename);
    else if(0==strcmp(argv[1], "test")) test_classifier(data, cfg, weights, layer);
    else if(0==strcmp(argv[1], "label")) label_classifier(data, cfg, weights);
    else if(0==strcmp(argv[1], "valid")) validate_classifier_single(data, cfg, weights);
    else if(0==strcmp(argv[1], "validmulti")) validate_classifier_multi(data, cfg, weights);
    else if(0==strcmp(argv[1], "valid10")) validate_classifier_10(data, cfg, weights);
    else if(0==strcmp(argv[1], "validcrop")) validate_classifier_crop(data, cfg, weights);
    else if(0==strcmp(argv[1], "validfull")) validate_classifier_full(data, cfg, weights);
}
